...
__kernel void fp_FFTlocal(__global fComplex *buffer,int M,__local fComplex *block)
{
	...
	for (uint loopidx = 0; loopidx < M; loopidx++)
	{
		...
		float rad = -PI * t / dleng;//invÇÃèÍçáÇ±ÇÍÇ…-ÇÇ©ÇØÇÈ
	...
	barrier(CLK_LOCAL_MEM_FENCE);
	fComplex reim0 = block[reversebits(id * 2) >> (32 - M)];//32ÇÕuint=32bitÇÃ32
...
__kernel void dp_FFTlocal(__global Complex *buffer,int M,__local Complex *block)
{
	...
	for (uint loopidx = 0; loopidx < M; loopidx++)
	{
		...
		double rad = -PI * t / dleng;//invÇÃèÍçáÇ±ÇÍÇ…-ÇÇ©ÇØÇÈ
	...
	barrier(CLK_LOCAL_MEM_FENCE);
	Complex reim0 = block[reversebits(id * 2) >> (32 - M)];//32ÇÕuint=32bitÇÃ32
