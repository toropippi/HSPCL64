...
２つ以上のデバイス上で同じカーネルを実行したいとき、それぞれのデバイスidをHCLSetDeviceでセットしなおしてHCLBuildProgramを実行して下さい。
...
例えばカーネル側のソースが「__kernel void vector_add(__global int *array1,int arg2) {}」というものなら
...
int p1,int p2,,,,,
int p1 : カーネルid			[in]
int p2以降 : 引数に渡す実体(定数やmem_object)	[in]

%inst
カーネルの引数をまとめて指定します。

HCLSetKernelの「引数の順番p(x)の指定」が0ならp2,1ならp3にあたります。
ローカルメモリのサイズ指定はできません。
...
HCLCreateProgram,HCLCreateKernel,HCLSetKernel,HCLCreateBuffer等をせずカーネルを一気に実行してします。
p1はソースコードの文字列
p2にはグローバルサイズ（実行したい並列処理数）
p3にはローカルサイズ
p4以降はカーネルに渡す引数を指定して下さい。

p4以降の引数の数とOpenCLカーネル内の引数の数が合わないとエラーが発生します。
例えばOpenCLで配列に値を書き込む処理をすると、p4以降に指定したHSP配列変数に結果が書き込まれます。
...
そのためにOpenCLを利用する上での独特な処理を説明します。
	...
	3.カーネル関数への引数をセットする際、その引数は　CL_mem_object idという固有のオブジェクト形式である必要がある。CL_mem_object　idは64bit int型の数値である。
...
プログラマはOpenCLでデータ並列処理を行うためにインデクス空間の次元数、ワークサイズの総数、ローカルサイズを指定することでインデクス空間を定義しなければなりません。
